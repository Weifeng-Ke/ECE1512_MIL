...
class ABMIL(nn.Module):
    def __init__(self, conf, D=128, droprate=0):
        ...
        self.pooling = getattr(conf, 'pooling', 'mean')
    ...
    def forward(self, x): ## x: N x L -> (B,N,L) = (1,N,D_feat) Batch,Number of patches/instance per bag, feature dimension
        ...
        ''' commentting out the original set up
        ...
        '''Adding this section to incoperate pooling instead of attention'''
        # Pool over instances instead of attention
        if self.pooling == 'mean':
            pooled = torch.mean(med_feat, dim=0, keepdim=True)  # (1, L=D_inner)
        elif self.pooling == 'max':
            pooled, _ = torch.max(med_feat, dim=0, keepdim=True)  # (1, L=D_inner)
        #outputs dimension Linear(128 -> n_class)-> logits(1,C)=(1,number of classes)
